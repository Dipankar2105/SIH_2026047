...
def test_appointment_double_booking(db):
    ...
    # Pick a unique future slot using a random offset to avoid collisions with prior runs
    random_day_offset = 100 + int(uuid.uuid4().int % 200)
    slot_time = (datetime.now(timezone.utc) + timedelta(days=random_day_offset)).replace(
