...
def create_kiosk_session(db: Session, session_in: KioskSessionCreate) -> KioskSession:
    ...
    # Create associated core Session for intake, prescriptions, and summaries
    db_session = DbSession(
        patient_id=session_in.patient_id,
        session_token=f"kiosk-{uuid.uuid4()}",
        status="active",
        started_at=now,
    )
    db.add(db_session)
...
def end_kiosk_session(db: Session, session_id: uuid.UUID) -> KioskSessionEndResponse:
    ...
    if session.session_id:
        parent_session = db.query(DbSession).filter(DbSession.id == session.session_id).first()
        if parent_session:
            parent_session.status = "ended"
            parent_session.ended_at = now
