...
def verify_patient_access(current_user: Dict[str, Any], patient_id: str | uuid.UUID, db: Session = None) -> None:
    """
    Prevents horizontal privilege escalation.
    Patients can only access their own data or data of their dependents.
    ...
    if user_role == "patient" and current_patient_id != target_patient_id:
        if db:
            ...
            is_dependent = db.query(FamilyMember).filter(
                FamilyMember.primary_patient_id == current_patient_id,
                FamilyMember.dependent_patient_id == target_patient_id
            ).first()
            if is_dependent:
                return
