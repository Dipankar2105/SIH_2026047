...
from app.models.session import Session as CoreSession
...
def seed_consents(db, patients: List[Patient]):
    now = datetime.now()
    for patient in patients:
        consent = db.query(Consent).filter_by(patient_id=patient.id, consent_type="health_record_sharing").first()
        if not consent:
            consent = Consent(
                patient_id=patient.id,
                consent_type="health_record_sharing",
                granted=True,
                purpose="Unified Health Record and Kiosk Access",
                scope="view_records",
                granted_at=now,
...
def seed_documents(db, patients: List[Patient]):
    sample_docs = [
        {
            "document_type": "lab_report",
            "file_name": "Blood_Glucose_Fast_PP.pdf",
            "storage_path": "https://storage.medikiosk.in/demo/Blood_Glucose_Fast_PP.pdf",
            "mime_type": "application/pdf",
            "ocr_text": "Lal PathLabs. Test: Fasting Blood Sugar: 98 mg/dL (Normal). Post Prandial Blood Sugar: 135 mg/dL (Normal). HbA1c: 5.6%.",
            "ocr_data": {
                "lab_name": "Lal PathLabs",
                "parameters": [
                    {"name": "Fasting Blood Sugar", "value": "98 mg/dL", "status": "normal"},
                    {"name": "Post Prandial Blood Sugar", "value": "135 mg/dL", "status": "normal"},
        ...
        },
        {
            ...
            "storage_path": "https://storage.medikiosk.in/demo/Chest_XRay_PA.pdf",
            "mime_type": "application/pdf",
            "ocr_text": "Apollo Radiology. Modality: Chest X-Ray PA View. Lung fields appear clear. Cardiothoracic ratio normal. No active focal lesions seen.",
            "ocr_data": {
                "modality": "X-Ray",
    ...
    for i, patient in enumerate(patients[:2]):
        doc_info = sample_docs[i % len(sample_docs)]
        doc = db.query(Document).filter_by(patient_id=patient.id, file_name=doc_info["file_name"]).first()
        if not doc:
            doc = Document(
                patient_id=patient.id,
                ...
                ocr_text=doc_info["ocr_text"],
                ocr_data=doc_info["ocr_data"],
                status="processed",
            )
            db.add(doc)
    db.commit()
...
def seed_summaries(db, patients: List[Patient], doctors: List[Doctor]):
    for i, patient in enumerate(patients[:2]):
        doctor = doctors[i % len(doctors)]
        summary = db.query(Summary).filter_by(patient_id=patient.id, doctor_id=doctor.id).first()
        if not summary:
            summary = Summary(
                patient_id=patient.id,
                doctor_id=doctor.id,
                summary_type="clinical",
                summary_text=f"Clinical Consultation Summary for {patient.first_name} {patient.last_name}.\n"
                             f"Diagnosis: Stable vitals with mild seasonal allergic rhinitis.\n"
                             f"Plan: Hydration, prescribed antihistamine course. Review in 1 week.",
                status="finalized",
            )
            db.add(summary)
    db.commit()
...
def seed_fhir_bundles(db, patients: List[Patient]):
    for patient in patients[:2]:
        bundle_rec = db.query(FHIRBundle).filter_by(patient_id=patient.id).first()
        if not bundle_rec:
            bundle_dict = {
                "resourceType": "Bundle",
                "id": str(uuid.uuid4()),
...
def seed_kiosk_sessions(db, patients: List[Patient]):
    now = datetime.now()
    for i, patient in enumerate(patients[:2]):
        kiosk_sess = db.query(KioskSession).filter_by(kiosk_id=f"KIOSK-BLR-{i+1:02d}").first()
        if not kiosk_sess:
            core_sess = CoreSession(
                patient_id=patient.id,
                session_token=f"demo-kiosk-token-{uuid.uuid4()}",
                status="completed",
            ...
            kiosk_sess = KioskSession(
                kiosk_id=f"KIOSK-BLR-{i+1:02d}",
                session_id=core_sess.id,
                status="completed",
                started_at=now - timedelta(hours=2),
                ended_at=now - timedelta(hours=1, minutes=45),
            )
...
def run_seed():
    ...
    try:
        ...
        seed_consents(db, patients)
        print(f"[OK] Seeded Active Consents for Demo Patients")

        seed_documents(db, patients)
        print(f"[OK] Seeded Digital Health Locker OCR Documents")

        seed_summaries(db, patients, doctors)
        print(f"[OK] Seeded Clinical Summaries")

        seed_fhir_bundles(db, patients)
        print(f"[OK] Seeded ABDM FHIR R4 Bundles")

        seed_kiosk_sessions(db, patients)
        print(f"[OK] Seeded Kiosk Sessions")
        
        print("\n[SUCCESS] Seed complete! Full demo data ecosystem is ready.")
